---
name: context-budget
description: Minimize input-token cost during coding, debugging, review, and investigation by packing many actions into each turn and hard-capping the bytes any tool may return, without skipping required correctness checks.
---
...
**Speculative chains.** For any task with predictable steps, silently predict the whole action sequence up front (like pre-planning "create A → import it in B → wire tests"), then execute the chain as batched calls. Continue the chain while each next action is confirmed by previous results; on the first surprise — an unexpected result, a failed assumption, a missing file — fall back to single-action mode and re-plan. Never keep firing batched actions on stale assumptions.
...
- **Long code?** Chunks go back to back in the same turn, no text between them.
...
- **Never into context:** full logs, build output, lockfiles, generated or minified files, dependency trees, directory listings over ~50 entries, binary/base64 data, JSON dumps over ~100 lines (select fields with `jq` instead). No single result over ~10k tokens, ever.
...
If honest discovery would pull more than ~3 files or ~400 lines into the main thread, delegate it to a search subagent instead. The subagent burns its own throwaway context and returns only a conclusion.
...
- Delegate when the result is a **fact or location** ("where is X validated", "which tests cover this"). Do the work yourself when the result is **content you will edit** — a summary cannot substitute for the actual lines you must patch.
- One subagent answering one focused question beats three agents in parallel that each re-scan the repo.

## Complete Work In Your Head, Then Emit

Planning is free; intermediate turns are not. Before the first write call, decide everything: every file's final content, imports, signatures, call sites, config, tests. Do not start writing to "see how it looks", and do not write half a design to discover the rest.
...
Final answer only, once, compact: what was done (1–2 sentences), changed files as `path:line` list, verification result (command + pass/fail), blockers or open questions if any. No restated task, no narration of steps already taken, no code blocks unless the user must copy or inspect them.
